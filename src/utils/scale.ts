...
export function scale(min: number, max: number, count: number): number[] {
  ...
  // Choose multiplier candidates. For integer input ranges with a magnitude >= 1,
  // avoid fractional multipliers like 2.5 so ticks remain integer. For fractional
  // magnitudes or non-integer endpoints, include 2.5 to allow quarter/half steps.
  const useFractionalMultiplier =
    !(Number.isInteger(min) && Number.isInteger(max) && magnitude >= 1);
  const niceMultipliers = useFractionalMultiplier
    ? [1, 2, 2.5, 5, 10]
    : [1, 2, 5, 10];
